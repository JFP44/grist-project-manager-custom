...
async function loadSettings() {
  try {
    ...
    // RACI désactivé dans cette version personnalisée.
    raciEnabled = false;
...
var SERVICES_TABLE = 'PM_Services';
...
function buildServiceDemandeurOptions(currentValue) {
  ...
  // Conserver une ancienne valeur si elle existe dans Grist
  // mais n'est plus présente dans la liste actuelle.
  if (currentValue && services.indexOf(currentValue) === -1) {
    html += '<option value="' +
  ...
  // Load project reference tables
    ...
    if (PROJECTS_TABLE === DEFAULT_PROJECTS_TABLE && existingTables.indexOf(PROJECTS_TABLE) === -1) {
      await grist.docApi.applyUserActions([
        ['AddTable', PROJECTS_TABLE, [
          ...
          { id: 'Description', type: 'Text' },
          { id: 'Color', type: 'Text' },
          { id: 'Status', type: 'Choice', widgetOptions: JSON.stringify({ choices: [
            'A valider',
            "A l'étude",
            'En développement',
            'En test',
    ...
    if (existingTables.indexOf(CONFIG_TABLE) === -1) {
      ...
      var defaultConfig = [
        ...
        ['project_service_demandeur', PROJECTS_TABLE, 'SERVICE DEMANDEUR', 'SERVICE DEMANDEUR', false, 'SERVICE DEMANDEUR'],
        // Categories mapping
...
async function loadAllData() {
  ...
  try {
    var serviceData = await grist.docApi.fetchTable(SERVICES_TABLE);
    PM_SERVICES = [];
    if (serviceData && serviceData.id) {
      for (var i = 0; i < serviceData.id.length; i++) {
        var service = String(serviceData.Service ? serviceData.Service[i] : '').trim();
        if (service && PM_SERVICES.indexOf(service) === -1) {
          PM_SERVICES.push(service);
        }
      }
    }
  } catch (e) {
...
function renderProjectSelector() {
  ...
  // Filtre SERVICE DEMANDEUR
  var serviceDemandeurOptions = PM_SERVICES.map(function(s) {
    return { value: s, label: s };
  });

  html += buildFilterCombo(
    'serviceDemandeur',
    '— SERVICE DEMANDEUR —',
    serviceDemandeurOptions,
  ...
  html += buildFilterCombo('tag', '— Statut —', tagOptions, currentFilterTag, filterByTag);
...
function isTabAllowed(tabId) {
  ...
  var ownerTabs = ['team', 'stats', 'settings'];
...
function renderStatsView() {
  ...
  filteredProjects.forEach(function(p) {
    ...
    // Si une ancienne donnée contient plusieurs tags, on utilise le premier.
    if (value.indexOf(',') !== -1) {
      value = value.split(',')[0].trim();
    }

    if (Object.prototype.hasOwnProperty.call(statusCounts, value)) {
      statusCounts[value]++;
    }
  });

  var maxStatus = Math.max.apply(null,
    STATUTS.map(function(s) { return statusCounts[s] || 0; }).concat([1])
  );
  ...
  var serviceEntries = Object.entries(serviceCounts).sort(function(a, b) {
    return b[1] - a[1];
  });

  var maxService = serviceEntries.length > 0 ? serviceEntries[0][1] : 1;
  var serviceColors = [
    '#3b82f6',
    '#8b5cf6',
    '#ec4899',
  ...
  serviceEntries.forEach(function(entry, i) {
    var height = (entry[1] / maxService) * 160;

    serviceHtml += '<div class="chart-bar">';
    serviceHtml += '<span class="chart-bar-value">' + entry[1] + '</span>';
    serviceHtml += '<div class="chart-bar-fill" style="height:' + height + 'px;background:' + serviceColors[i % serviceColors.length] + '"></div>';
    serviceHtml += '<span class="chart-bar-label">' + sanitize(entry[0]) + '</span>';
    serviceHtml += '</div>';
  });
...
function openProjectViewModal(projectId) {
  ...
  // 3. AVANCEMENT
  html += '<div style="border:1px solid #cbd5e1;border-radius:10px;padding:18px 20px;background:#ffffff;">';
...
async function saveProject() {
  ...
  var serviceDemandeurEl = document.getElementById('project-service-demandeur');
  ...
  try {
    ...
    document.getElementById('project-color').value = '#6366f1';
    document.getElementById('project-status').value = 'A valider';
    var serviceElReset = document.getElementById('project-service-demandeur');
...
function openProjectModalForEdit(projectId) {
  ...
  var statusOptions = PM_STATUTS.length
    ? PM_STATUTS.map(function(status) { return status.name; })
    : [
        'A valider',
        "A l'étude",
        'En développement',
  ...
  html += '<div class="form-group"><label>SERVICE DEMANDEUR</label>';
  html += '<select id="inline-proj-service-demandeur" class="form-input">';
  html += buildServiceDemandeurOptions(proj.SERVICE_DEMANDEUR || '');
  html += '</select></div>';
  ...
  statusOptions.forEach(function(s) {
    html += '<option value="' + s + '"' + (proj.Status === s ? ' selected' : '') + '>' + s + '</option>';
  });
  html += '</select></div></div>';
  html += '</div>';
  html += '<div class="modal-footer">';
...
async function saveInlineProjectEdit(projectId) {
  ...
  setField(record, 'projects', 'serviceDemandeur', document.getElementById('inline-proj-service-demandeur').value || '');
  setField(record, 'projects', 'color', document.getElementById('inline-proj-color').value || '#6366f1');
  setField(record, 'projects', 'status', document.getElementById('inline-proj-status').value || 'A valider');
  try {
...
function renderSettingsCategoriesList() {
  ...
  if (PM_SERVICES.length === 0) {
    html = '<div style="text-align:center;color:#94a3b8;padding:20px;">' + (currentLang === 'fr' ? 'Aucun service' : 'No services') + '</div>';
  } else {
    html = '<div class="settings-chips">';
    PM_SERVICES.forEach(function(service) {
      html += '<span class="settings-chip">' + sanitize(service) + '</span>';
    });
    html += '</div>';
  }
  container.innerHTML = html;
...
async function saveColumnMapping() {
  try {
    ...
    var projectFields = ['name', 'description', 'color', 'status', 'serviceDemandeur'];
